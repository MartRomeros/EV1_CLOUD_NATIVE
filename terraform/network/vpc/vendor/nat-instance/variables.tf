# Copia local, parcheada, de franciscobrioneslavados/terraform-aws-nat-instance
# (tag v1.5.1). Único cambio real respecto al original: se agrega
# instance_profile_name para reusar un instance profile ya existente en vez
# de que el módulo cree su propio aws_iam_role/aws_iam_instance_profile —
# AWS Academy no otorga permiso iam:CreateRole, y el módulo original no
# tenía ninguna variable para evitar esa creación. Ver README.md del
# proyecto para más contexto.
...
variable "instance_profile_name" {
  description = "Nombre de un instance profile ya existente (ej. el asociado a LabRole en AWS Academy) para SSM. Reemplaza al aws_iam_instance_profile que el módulo original creaba."
  type        = string
}
